...
public class ParseException extends Exception {
  ...
  public String getMessage() {
    ...
    ArrayList<String> listaNaoRepete = new ArrayList();
    ...
    for (int i = 0; i < expectedTokenSequences.length; i++) {
      ...
      for (int j = 0; j < expectedTokenSequences[i].length; j++) {
        if(!listaNaoRepete.contains(tokenImage[expectedTokenSequences[i][j]])){
          listaNaoRepete.add(tokenImage[expectedTokenSequences[i][j]]);
        }
        else repetiu = true;
      }
      if(!repetiu){
    ...
    listaNaoRepete.removeLast();
    listaNaoRepete.add(".");
    for(String str:listaNaoRepete){
      expected.append(str);
    }
    ...
    if (expectedTokenSequences.length == 1) {
      ...
      retval += "Estava esperando um dos tokens a seguir:" + "\n    ";
    }
